...
test('parses package identity from aapt badging output', () => {
  assert.deepEqual(
    parseAaptBadging("package: name='com.wjy.kaogong' versionCode='11' versionName='0.8.0' platformBuildVersionName='16'"),
    EXPECTED,
  );
});
...
test('valid source replaces stale APKs with one canonical embedded artifact', (t) => {
  ...
  fs.writeFileSync(source, 'version 0.8.0');
  ...
  assert.equal(fs.readFileSync(prepared.filePath, 'utf8'), 'version 0.8.0');
  assert.deepEqual(
