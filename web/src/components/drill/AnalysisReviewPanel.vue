  ...
  <section class="review-panel">
    ...
    <div v-if="result" class="result-stack">
      <div class="card result-heading">
        ...
        <div><span>识别题型</span><strong>{{ result.categoryLabel }}</strong></div>
      </div>
...
import { analyzeDataQuestion, type AnalysisCoachResult } from '../../lib/analysisCoach';
...
function runAnalysis() {
  ...
  if (!text) return;
  result.value = analyzeDataQuestion(text, userAnswer.value.trim(), correctAnswer.value.trim());
  store.saveAnalysisReview({ source: source.value, questionText: text, userAnswer: userAnswer.value.trim(), correctAnswer: correctAnswer.value.trim(), categoryKey: result.value.categoryKey, categoryLabel: result.value.categoryLabel, sections: result.value.sections });
  if ('vibrate' in navigator) navigator.vibrate?.(16);
}

function openReview(review: AnalysisReviewRecord) {
  source.value = review.source; questionText.value = review.questionText; userAnswer.value = review.userAnswer; correctAnswer.value = review.correctAnswer;
