  ...
  <section ref="panelEl" class="focus-panel">
    ...
    <div v-else class="question-stage">
      ...
      <article v-if="current" class="card focus-question">
        ...
        <h2>{{ current.stem }}</h2>
        <div v-if="current.titleImages.length" class="question-images">
          <QuestionImage v-for="(image, imageIndex) in current.titleImages" :key="image" :src="image" :alt="`第 ${index + 1} 题题图 ${imageIndex + 1}`" />
        </div>
        ...
        <div class="options" aria-label="答案选项">
          <button v-for="(option, optionIndex) in visibleOptions" :key="`${current.id}-${optionIndex}`" class="option" :class="{ selected: answer === optionLetter(optionIndex), correct: answered && optionLetter(optionIndex) === current.answer, wrong: answered && answer === optionLetter(optionIndex) && optionLetter(optionIndex) !== current.answer }" type="button" :disabled="answered" @click="selectAnswer(optionIndex)">
            <span>{{ optionLetter(optionIndex) }}</span>
            <strong v-if="option">{{ option }}</strong>
            <strong v-else-if="!current.optionImages[optionIndex]?.length">原题未提供选项文字</strong>
            <div v-if="current.optionImages[optionIndex]?.length" class="option-images">
              <QuestionImage v-for="(image, imageIndex) in current.optionImages[optionIndex]" :key="image" :src="image" :alt="`选项 ${optionLetter(optionIndex)} 图片 ${imageIndex + 1}`" />
            </div>
          </button>
        </div>
...
import { ANALYSIS_BANK_COUNT, loadAnalysisQuestionBank, searchAnalysisQuestionBank, type AnalysisBankQuestion } from '../../lib/questionBank';
...
const visibleOptions = computed(() => current.value?.options || []);
...
async function startRandom() {
  ...
  const bank = searchAnalysisQuestionBank('', '', bankCount);
  if (bank.length) startQuestion(bank[Math.floor(Math.random() * bank.length)]);
}
function selectAnswer(optionIndex: number) { if (!answered.value) answer.value = optionLetter(optionIndex); }
function submitAnswer() {
  if (!current.value || !answer.value || answered.value) return;
  const correct = answer.value.trim().toUpperCase() === current.value.answer.trim().toUpperCase();
