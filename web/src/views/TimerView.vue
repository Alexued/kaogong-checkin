...
const sessionActive = computed(() => mode.value === 'countdown' ? !!countdown.startedAt : !!sw.startedAt);
const isRunning = computed(() => mode.value === 'countdown' ? countdown.running : sw.running);
const remainingDisplay = ref(countdown.remainingMs());
const clockText = ref(mode.value === 'countdown' ? fmtCountdown(remainingDisplay.value) : fmtClock(0));
...
function tick() {
  ...
  if (sw.running || countdown.running) raf = requestAnimationFrame(tick);
}
onMounted(() => { if (sw.running || countdown.running) raf = requestAnimationFrame(tick); });
